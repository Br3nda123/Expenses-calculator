...
const ligthStyleBtn = document.querySelector('.light');
...
const checkForm = () => {
   if (nameInput.value !== '' && amoutInput.value !== '' && categorySelect.value !== 'none') {
      creatNewTransaction();
   } else {
...
const creatNewTransaction = () => {
   ...
   amoutInput.value > 0 ? incomeSection.appendChild(newTransaction) && newTransaction.classList.add('income') : expensesSection.appendChild(newTransaction) && newTransaction.classList.add('expense');

   moneyArr.push(Number(amoutInput.value));
   countMoney(moneyArr);
   closePanel();
   ID++;
};
...
const selectCategory = () => {
   selectedCategory = categorySelect[categorySelect.selectedIndex].text;
};
...
ligthStyleBtn.addEventListener('click', changeStyleToLight);
